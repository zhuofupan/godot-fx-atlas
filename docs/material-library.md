...
- **生产贴图库**：只保存许可允许再分发、来源被锁定、并经过安全清洗的位图。每个文件都有用途标签、尺寸、哈希和来源收据。

这使 Atlas 可以吸收成熟资源的价值，但不会把未知插件代码、许可不明素材或付费资源直接复制进仓库。
...
即使来源允许直接使用，也只导入清洗后的位图。脚本、二进制、编辑器插件、Shader 和项目文件不会跟随素材包进入 `materials/library/`。原始压缩包与隔离缓存保存在仓库之外。
